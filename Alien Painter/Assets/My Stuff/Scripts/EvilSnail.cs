...
public class EvilSnail : MonoBehaviour
{

    [SerializeField] float snailSpeed = 1.0f;
    ...
    void Update()
    {
        ...
        if (flipTimer <= 0 && !( Physics2D.Raycast(new Vector3(transform.position.x + .6f, transform.position.y - .5f, transform.position.z), Vector2.down, raylegnth, LayerMask.GetMask("Ground"))) )
        {
            Flip();
        }
        if (flipTimer <= 0 && !(Physics2D.Raycast(new Vector3(transform.position.x - .6f, transform.position.y - .5f, transform.position.z), Vector2.down, raylegnth, LayerMask.GetMask("Ground"))))
    ...
    private void GoLeft()
    {
        ...
        rbody.velocity = new Vector2(direction.x, direction.y).normalized * snailSpeed;
        isLeft = true;
    }
